...
known_faces = {
    "Salim": DeepFace.represent(img_path="salim.jpg", model_name="VGG-Face", enforce_detection=False)[0]["embedding"],
    "Chedly": DeepFace.represent(img_path="chedly.jpg", model_name="VGG-Face", enforce_detection=False)[0]["embedding"]
}
threshold = 0.75
...
def face_login():
    ...
    try:
        ...
        if len(faces) == 0:
            return jsonify({"status": "failed"})  # No face detected
        ...
        for face in faces:
            ...
            for name, ref_embedding in known_faces.items():
                ...
                if dist < threshold:
                    session['user'] = name  # ✅ remember who logged in
